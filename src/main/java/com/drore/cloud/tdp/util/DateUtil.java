...
//TODO////////////java8//////////
    ...
    public static String getNowTime() {
        DateTimeFormatter f = getDateFormat(YMD_DASH_WITH_TIME);
    ...
    public static JSONArray getDayHour(int start, int end) {
        JSONArray timeArray = new JSONArray();
        Date dateHour = new Date();
        Integer hourNum = Integer.parseInt(date2String(dateHour, "HH"));
        int index = 0;
        for (int i = hourNum - start; i >= 0 + index; i--) {
            if (hourNum > end) {
                index = hourNum - end;
            }
            ...
            Date date = c.getTime();
            String dateStr = date2String(date, "HH");
     ...
     * @return
     */
    public static String getFiveMinutePoint() {
        ...
        try {
            time = sfEnd.format(sfStart.parse(time).getTime() - (5 * 60 * 1000));
        } catch (ParseException e) {
            e.printStackTrace();
        }
        return time;
...
//        String startTime = "2017-10-11 13:50:59";
//        startTime = DateUtil.format(DateUtil.add(DateUtil.parse(startTime, DateUtil.YMD_DASH_WITH_TIME), Calendar.SECOND, 1));
